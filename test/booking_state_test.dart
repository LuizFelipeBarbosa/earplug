...
void main() {
  group('booking state', () {
    testWidgets(
      'booking and dispute loads wait for the restored session token',
      (tester) async {
    ...
    testWidgets('loadBooking caches and refresh forces a re-fetch', (
      ...
      final refreshed = await harness.app.loadBooking(
        booking.id,
        refresh: true,
      );
    ...
    testWidgets('loadBooking adopts the organizer without navigating', (
      ...
      final booking = await harness.app.loadBooking(
        repository.bookingResult.id,
      );
    ...
    testWidgets('loadBooking adopts the band without navigating', (
      tester,
    ) async {
      final auth = FakeAuthService();
      final repository = _ControlledBookingRepository(auth: auth)
    ...
    testWidgets('admin dispute actions forward their arguments', (
      tester,
    ) async {
      final auth = FakeAuthService();
      final repository = _ControlledBookingRepository(auth: auth)
