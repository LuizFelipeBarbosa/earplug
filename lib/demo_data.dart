import 'package:flutter/material.dart';
import 'package:latlong2/latlong.dart';

import 'models.dart';

/// Demo dataset lifted verbatim from the design spec. This is the seam where
/// Convex queries will plug in later — screens only ever see these shapes.
abstract final class DemoData {
  static const demoUserId = 'demo-user';

  static const venues = <String, Venue>{
    'v1': Venue(
      id: 'v1',
      name: 'The Foghorn Club',
      area: 'Mission, San Francisco',
      addr: 'Mission, San Francisco',
      point: LatLng(37.7599, -122.4148),
      slug: 'the-foghorn-club',
      description: 'An intimate Mission bar with a welcoming live room.',
      venueType: VenueType.bar,
      capacityPublic: 180,
      approx: ApproxLocation(
        centroid: LatLng(37.7599, -122.4148),
        label: 'Mission, San Francisco',
      ),
      neighborhood: 'Mission',
      city: 'San Francisco',
      disclosure: AddressDisclosure.onTicket,
      verified: true,
      managedByOrganizationId: 'org1',
      supportsApproxLocation: true,
    ),
    'v2': Venue(
      id: 'v2',
      name: 'Nightcrawler Records',
      area: 'Temescal, Oakland',
      addr: '486 40th St, Oakland',
      point: LatLng(37.8180, -122.2690),
    ),
    'v3': Venue(
      id: 'v3',
      name: 'Casa Quake',
      area: 'Bernal Heights, SF',
      addr: '(address with RSVP)',
      point: LatLng(37.7399, -122.4166),
    ),
    'v4': Venue(
      id: 'v4',
      name: 'The Rusty Anchor',
      area: 'Dogpatch, SF',
      addr: '701 22nd St, San Francisco',
      point: LatLng(37.7583, -122.3880),
    ),
    'v5': Venue(
      id: 'v5',
      name: 'Peralta Hall',
      area: 'West Oakland',
      addr: '1801 Peralta St, Oakland',
      point: LatLng(37.8060, -122.2910),
    ),
    'v6': Venue(
      id: 'v6',
      name: 'Sunset Bunker',
      area: 'Outer Sunset, SF',
      addr: '3820 Noriega St, San Francisco',
      point: LatLng(37.7540, -122.5040),
    ),
  };

  static const venuePrivateDetails = <String, VenuePrivateDetails>{
    'v1': VenuePrivateDetails(
      venueId: 'v1',
      addr: '2455 Harrison St, San Francisco',
      point: LatLng(37.7524, -122.4180),
      loadInNotes: 'Use the side entrance for load-in.',
      capacity: 180,
    ),
  };

  static final privateLocations = <String, PrivateLocation>{
    'private-location-org2': PrivateLocation(
      id: 'private-location-org2',
      organizationId: 'org2',
      label: "Jordan's courtyard",
      addr: '120 Demo Lane, San Francisco',
      city: 'San Francisco',
      area: 'Mission District',
      lat: 37.7599,
      lng: -122.4148,
      notes: 'Use the side gate for load-in.',
      createdAt: DateTime(2025, 6, 1),
      updatedAt: DateTime(2025, 6, 1),
    ),
  };

  static final organizations = <String, Organization>{
    'org3': Organization(
      id: 'org3',
      slug: 'night-shift-collective',
      name: 'Night Shift Collective',
      orgType: OrganizationType.promoter,
      status: OrganizationStatus.verified,
      verified: true,
      photoUrls: const [],
      createdAt: DateTime(2025, 7, 1),
    ),
    'org2': Organization(
      id: 'org2',
      slug: 'jordan-host',
      name: 'Jordan (host)',
      orgType: OrganizationType.privateHost,
      status: OrganizationStatus.verified,
      verified: true,
      photoUrls: const [],
      createdAt: DateTime(2025, 6, 1),
    ),
    'org1': Organization(
      id: 'org1',
      slug: 'the-foghorn-club',
      name: 'The Foghorn Club',
      orgType: OrganizationType.venueOperator,
      status: OrganizationStatus.verified,
      verified: true,
      description: 'Independent Mission venue and community room.',
      website: 'https://example.com/the-foghorn-club',
      photoUrls: const [],
      createdAt: DateTime(2025, 1, 15),
      reviewSummary: const ReviewSummary(
        count: 1,
        mean: 4,
        completedBookings: 1,
        cancellations: 0,
      ),
    ),
  };

  static final organizationMembers = <String, OrganizationMember>{
    demoUserId: OrganizationMember(
      userId: demoUserId,
      name: 'Earplug Fan',
      email: 'fan@example.com',
      role: OrganizationRole.owner,
      createdAt: DateTime(2025, 1, 15),
    ),
    'manager1': OrganizationMember(
      userId: 'manager1',
      name: 'Mara Kim',
      email: 'mara@example.com',
      role: OrganizationRole.manager,
      createdAt: DateTime(2025, 2, 1),
    ),
  };

  static final submittedOrganizationApplication = OrganizationApplication(
    id: 'application-review-1',
    status: OrganizationApplicationStatus.submitted,
    orgName: 'The Knockout',
    orgType: OrganizationType.venueOperator,
    website: 'https://example.com/the-knockout',
    contactName: 'Sam Reyes',
    businessEmail: 'sam@example.com',
    phone: null,
    venue: const ApplicationVenueDraft(
      name: 'The Knockout',
      addr: '3223 Mission St, San Francisco',
      point: LatLng(37.7452, -122.4201),
      area: 'Mission, San Francisco',
      neighborhood: 'Mission',
      city: 'San Francisco',
      capacity: 140,
      venueType: VenueType.bar,
    ),
    documents: const [],
    reviewNote: null,
    decidedAt: null,
    resultingOrganizationId: null,
    resultingVenueId: null,
    revision: 1,
    createdAt: DateTime(2026, 8, 20),
    updatedAt: DateTime(2026, 8, 20),
  );

  static final adminApplicationRows = <AdminApplicationRow>[
    AdminApplicationRow(
      application: submittedOrganizationApplication,
      applicantUserId: 'applicant-sam',
      applicantName: 'Sam Reyes',
      applicantEmail: 'sam@example.com',
    ),
  ];

  static final _demoToday = () {
    final now = DateTime.now();
    return DateTime(now.year, now.month, now.day);
  }();

  static DateTime _demoStartsAt(int daysFromToday, int hour) => DateTime(
    _demoToday.year,
    _demoToday.month,
    _demoToday.day + daysFromToday,
    hour,
  );

  static String _demoDateShort(int daysFromToday, int hour) => Gig.dateShortFor(
    _demoStartsAt(daysFromToday, hour).millisecondsSinceEpoch,
  );

  static String _demoDateLine(int daysFromToday, int hour, String doorsTime) =>
      Gig.dateLineFor(
        _demoStartsAt(daysFromToday, hour).millisecondsSinceEpoch,
        doorsTime,
        now: _demoToday,
      );

  static final opportunitySlots = <String, List<OpportunitySlot>>{
    'opp-promoter': [
      OpportunitySlot(
        id: 'opp-promoter-headliner',
        order: 0,
        role: SlotRole.headliner,
        guaranteeMinor: 0,
        required: true,
        status: SlotStatus.open,
      ),
    ],
    'opp-private': [
      OpportunitySlot(
        id: 'opp-private-headliner',
        order: 0,
        role: SlotRole.headliner,
        guaranteeMinor: 25000,
        required: true,
        status: SlotStatus.open,
      ),
    ],
    'opp1': [
      OpportunitySlot(
        id: 'opp1-headliner',
        order: 0,
        role: SlotRole.headliner,
        guaranteeMinor: 30000,
        required: true,
        status: SlotStatus.open,
      ),
      OpportunitySlot(
        id: 'opp1-support',
        order: 1,
        role: SlotRole.support,
        guaranteeMinor: 15000,
        required: true,
        status: SlotStatus.open,
      ),
    ],
    'opp2': [
      OpportunitySlot(
        id: 'opp2-opener',
        order: 0,
        role: SlotRole.opener,
        guaranteeMinor: 0,
        required: true,
        status: SlotStatus.open,
      ),
    ],
    'opp3': [
      OpportunitySlot(
        id: 'opp3-headliner',
        order: 0,
        role: SlotRole.headliner,
        guaranteeMinor: 0,
        required: true,
        status: SlotStatus.open,
      ),
    ],
  };

  /// Upcoming bookings and deadlines stay useful whenever the demo is opened.
  static final opportunities = <String, Opportunity>{
    'opp-promoter': Opportunity(
      id: 'opp-promoter',
      organizationId: 'org3',
      mode: OpportunityMode.publicEvent,
      venueId: 'v1',
      venue: venues['v1'],
      title: 'Night Shift at The Foghorn Club',
      desc: 'A late-night showcase of local bands from Night Shift Collective.',
      genres: List<String>.of(bands['b1']!.genres),
      startsAt: _demoStartsAt(40, 21),
      ageRequirement: AgeRequirement.allAges,
      flyKey: 'xerox',
      applicationsCloseAt: _demoStartsAt(33, 21),
      visibility: OpportunityVisibility.publicListing,
      ticketing: OpportunityTicketing.none,
      status: OpportunityStatus.draft,
      slug: 'night-shift-at-the-foghorn-club',
      revision: 1,
      applicationCount: 0,
      slots: opportunitySlots['opp-promoter']!,
      invitedBandIds: const [],
      createdAt: _demoToday.subtract(const Duration(days: 1)),
      updatedAt: _demoToday.subtract(const Duration(days: 1)),
      area: venues['v1']!.area,
      venueType: venues['v1']!.venueType,
      currency: 'usd',
    ),
    'opp-private': Opportunity(
      id: 'opp-private',
      organizationId: 'org2',
      mode: OpportunityMode.privateBooking,
      privateEvent: true,
      privateLocationId: 'private-location-org2',
      title: 'Courtyard birthday set',
      desc: 'A relaxed evening of live music for friends and family.',
      genres: List<String>.of(bands['b1']!.genres),
      startsAt: _demoStartsAt(30, 18),
      ageRequirement: AgeRequirement.allAges,
      flyKey: 'xerox',
      applicationsCloseAt: _demoStartsAt(23, 18),
      visibility: OpportunityVisibility.publicListing,
      ticketing: OpportunityTicketing.none,
      status: OpportunityStatus.open,
      slug: 'courtyard-birthday-set',
      revision: 1,
      applicationCount: 0,
      slots: opportunitySlots['opp-private']!,
      invitedBandIds: const [],
      createdAt: _demoToday.subtract(const Duration(days: 1)),
      updatedAt: _demoToday.subtract(const Duration(days: 1)),
      area: privateLocations['private-location-org2']!.area,
      venueType: VenueType.private,
      currency: 'usd',
    ),
    'opp1': Opportunity(
      id: 'opp1',
      organizationId: 'org1',
      mode: OpportunityMode.publicEvent,
      venueId: 'v1',
      venue: venues['v1'],
      title: 'Friday Night Live',
      desc: 'A loud local double bill in the Foghorn Club live room.',
      genres: List<String>.of(bands['b1']!.genres),
      startsAt: _demoStartsAt(12, 21),
      ageRequirement: AgeRequirement.allAges,
      flyKey: 'xerox',
      applicationsCloseAt: _demoStartsAt(5, 21),
      visibility: OpportunityVisibility.publicListing,
      ticketing: OpportunityTicketing.rsvp,
      status: OpportunityStatus.open,
      slug: 'friday-night-live',
      revision: 1,
      applicationCount: 2,
      slots: opportunitySlots['opp1']!,
      invitedBandIds: const [],
      createdAt: _demoToday.subtract(const Duration(days: 3)),
      updatedAt: _demoToday.subtract(const Duration(days: 2)),
      area: venues['v1']!.approx.label,
      venueType: venues['v1']!.venueType,
      currency: 'usd',
    ),
    'opp2': Opportunity(
      id: 'opp2',
      organizationId: 'org1',
      mode: OpportunityMode.publicEvent,
      venueId: 'v1',
      venue: venues['v1'],
      title: 'Patio Sessions',
      desc: 'An early evening set on the patio for neighborhood music fans.',
      genres: const [],
      startsAt: _demoStartsAt(20, 18),
      ageRequirement: AgeRequirement.allAges,
      flyKey: 'xerox',
      applicationsCloseAt: _demoStartsAt(13, 18),
      visibility: OpportunityVisibility.publicListing,
      ticketing: OpportunityTicketing.rsvp,
      status: OpportunityStatus.draft,
      slug: 'patio-sessions',
      revision: 1,
      applicationCount: 0,
      slots: opportunitySlots['opp2']!,
      invitedBandIds: const [],
      createdAt: _demoToday.subtract(const Duration(days: 2)),
      updatedAt: _demoToday.subtract(const Duration(days: 1)),
      area: venues['v1']!.approx.label,
      venueType: venues['v1']!.venueType,
      currency: 'usd',
    ),
    'opp3': Opportunity(
      id: 'opp3',
      organizationId: 'org1',
      mode: OpportunityMode.publicEvent,
      venueId: 'v1',
      venue: venues['v1'],
      title: 'Private Preview',
      desc: 'An invitation to try new songs with a small audience of friends.',
      genres: const [],
      startsAt: _demoStartsAt(15, 20),
      ageRequirement: AgeRequirement.allAges,
      flyKey: 'xerox',
      applicationsCloseAt: _demoStartsAt(8, 20),
      visibility: OpportunityVisibility.inviteOnly,
      ticketing: OpportunityTicketing.rsvp,
      status: OpportunityStatus.open,
      slug: 'private-preview',
      revision: 1,
      applicationCount: 0,
      slots: opportunitySlots['opp3']!,
      invitedBandIds: const ['b1'],
      createdAt: _demoToday.subtract(const Duration(days: 3)),
      updatedAt: _demoToday.subtract(const Duration(days: 1)),
      area: venues['v1']!.approx.label,
      venueType: venues['v1']!.venueType,
      currency: 'usd',
    ),
  };

  static final venueConsents = <String, VenueConsent>{
    'consent-1': VenueConsent(
      id: 'consent-1',
      opportunityId: 'opp-promoter',
      venueId: 'v1',
      venueOrganizationId: 'org1',
      requestingOrganizationId: 'org3',
      status: VenueConsentStatus.pending,
      createdAt: _demoToday.subtract(const Duration(hours: 6)),
    ),
  };

  static final artistApplications = <String, ArtistApplication>{
    'app1': ArtistApplication(
      id: 'app1',
      opportunityId: 'opp1',
      slotId: 'opp1-support',
      bandId: 'b1',
      status: ArtistApplicationStatus.submitted,
      message: 'We can bring 40 people.',
      createdAt: _demoToday.subtract(const Duration(days: 2)),
      updatedAt: _demoToday.subtract(const Duration(days: 2)),
    ),
    'app2': ArtistApplication(
      id: 'app2',
      opportunityId: 'opp1',
      slotId: 'opp1-headliner',
      bandId: 'b2',
      status: ArtistApplicationStatus.shortlisted,
      message: 'We have a full set ready and would love to headline.',
      createdAt: _demoToday.subtract(const Duration(days: 3)),
      updatedAt: _demoToday.subtract(const Duration(days: 2)),
    ),
  };

  /// Independent booking read models; their synthetic links deliberately do
  /// not add opportunities, applications, slots, or gigs to shared fixtures.
  static final bookings = <String, Booking>{
    'bk1': Booking(
      id: 'bk1',
      opportunityId: 'demo-opp-bk1',
      opportunityTitle: 'Sunset Sessions',
      opportunitySlug: 'sunset-sessions',
      slotId: 'demo-slot-bk1',
      slotRole: SlotRole.headliner,
      slotRequired: true,
      organizationId: 'org1',
      organizationName: 'The Foghorn Club',
      bandId: 'b2',
      bandName: 'Pigeon Court',
      bandSlug: '',
      applicationId: 'demo-app-bk1',
      status: BookingStatus.offerSent,
      revision: 1,
      startsAt: _demoStartsAt(25, 21),
      fee: const FeeBreakdown(
        grossMinor: 30000,
        commissionBps: 1000,
        commissionMinor: 3000,
        artistNetMinor: 27000,
        currency: 'usd',
      ),
      cancellationTemplate: CancellationTemplate.standard,
      organizerAcceptedTermsAt: _demoToday.subtract(const Duration(days: 1)),
      expiresAt: _demoToday.add(const Duration(days: 2)),
      currentOffer: BookingOffer(
        revision: 1,
        message: 'Excited to have you headline.',
        sentAt: _demoToday.subtract(const Duration(days: 1)),
        expiresAt: _demoToday.add(const Duration(days: 2)),
      ),
      venue: const BookingVenue(
        id: 'v1',
        name: 'The Foghorn Club',
        slug: 'the-foghorn-club',
      ),
      viewerSide: BookingSide.organizer,
    ),
    'bk2': Booking(
      id: 'bk2',
      opportunityId: 'demo-opp-bk2',
      opportunityTitle: 'Riverside Sessions',
      opportunitySlug: 'riverside-sessions',
      slotId: 'demo-slot-bk2',
      slotRole: SlotRole.headliner,
      slotRequired: true,
      organizationId: 'org1',
      organizationName: 'The Foghorn Club',
      bandId: 'b1',
      bandName: 'Foghorn Diet',
      bandSlug: 'foghorn-diet',
      applicationId: 'demo-app-bk2',
      status: BookingStatus.confirmed,
      revision: 3,
      startsAt: _demoStartsAt(18, 20),
      fee: const FeeBreakdown(
        grossMinor: 0,
        commissionBps: 0,
        commissionMinor: 0,
        artistNetMinor: 0,
        currency: 'usd',
      ),
      cancellationTemplate: CancellationTemplate.flexible,
      organizerAcceptedTermsAt: _demoToday.subtract(const Duration(days: 2)),
      artistAcceptedTermsAt: _demoToday.subtract(const Duration(days: 2)),
      confirmedAt: _demoToday.subtract(const Duration(days: 2)),
      publicGigId: 'demo-gig-bk2',
      publicGigSlug: 'riverside-sessions-live',
      venue: const BookingVenue(
        id: 'v1',
        name: 'The Foghorn Club',
        slug: 'the-foghorn-club',
      ),
      viewerSide: BookingSide.organizer,
    ),
    'bk3': Booking(
      id: 'bk3',
      opportunityId: 'demo-opp-bk3',
      opportunityTitle: 'Summer Closer',
      opportunitySlug: 'summer-closer',
      slotId: 'demo-slot-bk3',
      slotRole: SlotRole.headliner,
      slotRequired: true,
      organizationId: 'org1',
      organizationName: 'The Foghorn Club',
      bandId: 'b1',
      bandName: 'Foghorn Diet',
      bandSlug: 'foghorn-diet',
      applicationId: 'demo-app-bk3',
      status: BookingStatus.completed,
      revision: 4,
      startsAt: _demoToday.subtract(const Duration(days: 20)),
      fee: const FeeBreakdown(
        grossMinor: 0,
        commissionBps: 0,
        commissionMinor: 0,
        artistNetMinor: 0,
        currency: 'usd',
      ),
      cancellationTemplate: CancellationTemplate.flexible,
      organizerAcceptedTermsAt: _demoToday.subtract(const Duration(days: 20)),
      artistAcceptedTermsAt: _demoToday.subtract(const Duration(days: 20)),
      confirmedAt: _demoToday.subtract(const Duration(days: 20)),
      completedAt: _demoToday.subtract(const Duration(days: 19)),
      venue: const BookingVenue(
        id: 'v1',
        name: 'The Foghorn Club',
        slug: 'the-foghorn-club',
      ),
      viewerSide: BookingSide.organizer,
    ),
    'bk4': Booking(
      id: 'bk4',
      opportunityId: 'demo-opp-bk4',
      opportunityTitle: 'Late Night Wrap',
      opportunitySlug: 'late-night-wrap',
      slotId: 'demo-slot-bk4',
      slotRole: SlotRole.headliner,
      slotRequired: true,
      organizationId: 'org1',
      organizationName: 'The Foghorn Club',
      bandId: 'b2',
      bandName: 'Pigeon Court',
      bandSlug: '',
      applicationId: 'demo-app-bk4',
      status: BookingStatus.completed,
      revision: 4,
      startsAt: _demoToday.subtract(const Duration(days: 10)),
      fee: const FeeBreakdown(
        grossMinor: 0,
        commissionBps: 0,
        commissionMinor: 0,
        artistNetMinor: 0,
        currency: 'usd',
      ),
      cancellationTemplate: CancellationTemplate.standard,
      organizerAcceptedTermsAt: _demoToday.subtract(const Duration(days: 10)),
      artistAcceptedTermsAt: _demoToday.subtract(const Duration(days: 10)),
      confirmedAt: _demoToday.subtract(const Duration(days: 10)),
      completedAt: _demoToday.subtract(const Duration(days: 9)),
      venue: const BookingVenue(
        id: 'v1',
        name: 'The Foghorn Club',
        slug: 'the-foghorn-club',
      ),
      viewerSide: BookingSide.organizer,
    ),
  };

  /// Dates stay relative to app startup so every demo session has useful
  /// Tonight, This Week, and later results for discovery filtering.
  static final gigs = <Gig>[
    Gig(
      id: 'g1',
      title: 'Basement Blowout',
      venueId: 'v3',
      price: 0,
      startsAt: _demoStartsAt(0, 21),
      dateShort: _demoDateShort(0, 21),
      dateLine: _demoDateLine(0, 21, '8PM / 9PM'),
      time: '8PM / 9PM',
      when: GigWhen.tonight,
      flyKey: 'paper',
      lineup: ['b3', 'b4', 'b6'],
      going: 43,
      genres: ['hardcore', 'noise'],
      desc:
          'Four bands, one basement, zero cover. Bring earplugs (the foam kind).',
      tix: Ticketing.rsvp,
      ageRequirement: AgeRequirement.eighteenPlus,
    ),
    Gig(
      id: 'g2',
      title: 'Riptide Release Show',
      venueId: 'v1',
      price: 10,
      startsAt: _demoStartsAt(2, 21),
      dateShort: _demoDateShort(2, 21),
      dateLine: _demoDateLine(2, 21, '8PM / 9PM'),
      time: '8PM / 9PM',
      when: GigWhen.week,
      flyKey: 'blue',
      lineup: ['b1', 'b2'],
      going: 87,
      genres: ['garage', 'surf'],
      desc:
          "Foghorn Diet plays 'Riptide' front to back, then whatever falls out.",
      tix: Ticketing.rsvp,
      createdByBand: 'b1',
      discoveryListingReady: true,
    ),
    Gig(
      id: 'g3',
      title: 'Noise Night Vol. 12',
      venueId: 'v2',
      price: 8,
      startsAt: _demoStartsAt(3, 21),
      dateShort: _demoDateShort(3, 21),
      dateLine: _demoDateLine(3, 21, '9PM / 9:30PM'),
      time: '9PM / 9:30PM',
      when: GigWhen.week,
      flyKey: 'black',
      lineup: ['b4', 'b6'],
      going: 29,
      genres: ['noise', 'shoegaze'],
      desc: 'In-store racket between the record bins. BYO earplugs, again.',
      tix: Ticketing.rsvp,
      ageRequirement: AgeRequirement.eighteenPlus,
    ),
    Gig(
      id: 'g4',
      title: 'Pigeon Court + Mission Creep',
      venueId: 'v4',
      price: 12,
      startsAt: _demoStartsAt(4, 21),
      dateShort: _demoDateShort(4, 21),
      dateLine: _demoDateLine(4, 21, '8PM / 8:45PM'),
      time: '8PM / 8:45PM',
      when: GigWhen.week,
      flyKey: 'bluetype',
      lineup: ['b2', 'b3'],
      going: 56,
      genres: ['post-punk', 'hardcore'],
      desc: 'Double bill. Loud then louder. Tickets through the venue.',
      tix: Ticketing.external,
      ageRequirement: AgeRequirement.twentyOnePlus,
      externalUrl: 'https://example.com/tickets/pigeon-court',
    ),
    Gig(
      id: 'g5',
      title: 'All-Ages Matinee',
      venueId: 'v5',
      price: 0,
      startsAt: _demoStartsAt(5, 14),
      dateShort: _demoDateShort(5, 14),
      dateLine: _demoDateLine(5, 14, '1:30PM / 2PM'),
      time: '1:30PM / 2PM',
      when: GigWhen.week,
      flyKey: 'yellow',
      lineup: ['b5', 'b6', 'b3'],
      going: 61,
      genres: ['punk'],
      desc: 'Daylight punk. Kids welcome, chaperones tolerated.',
      tix: Ticketing.rsvp,
      ageRequirement: AgeRequirement.allAges,
    ),
    Gig(
      id: 'g6',
      title: 'Trash Panda Riot',
      venueId: 'v6',
      price: 5,
      startsAt: _demoStartsAt(10, 22),
      dateShort: _demoDateShort(10, 22),
      dateLine: _demoDateLine(10, 22, '9PM / 10PM'),
      time: '9PM / 10PM',
      when: GigWhen.later,
      flyKey: 'black',
      lineup: ['b5'],
      going: 18,
      genres: ['thrash', 'punk'],
      desc: 'One band, ninety minutes, a bunker by the beach.',
      tix: Ticketing.rsvp,
    ),
    Gig(
      id: 'g7',
      title: 'Fog City Fest — Day Show',
      venueId: 'v1',
      price: 15,
      startsAt: _demoStartsAt(12, 12),
      dateShort: _demoDateShort(12, 12),
      dateLine: _demoDateLine(12, 12, '12PM / 12:30PM'),
      time: '12PM / 12:30PM',
      when: GigWhen.later,
      flyKey: 'blue',
      lineup: ['b1', 'b2', 'b4', 'b5'],
      going: 112,
      genres: ['punk', 'garage'],
      desc: 'Eight hours, four stages worth of bands on one stage.',
      tix: Ticketing.external,
      externalUrl: 'https://example.com/tickets/fog-city-fest',
    ),
    Gig(
      id: 'g8',
      slug: 'paid-show-at-the-vault',
      title: 'Paid Show at the Vault',
      venueId: 'v4',
      price: 25,
      ticketPriceMinor: 2500,
      ticketCurrency: 'usd',
      startsAt: _demoStartsAt(14, 21),
      doorsAt: _demoStartsAt(14, 20),
      dateShort: _demoDateShort(14, 21),
      dateLine: _demoDateLine(14, 21, '8PM / 9PM'),
      time: '8PM / 9PM',
      when: GigWhen.later,
      flyKey: 'blue',
      lineup: ['b1', 'b2'],
      going: 0,
      genres: ['garage', 'post-punk'],
      desc: 'An intimate double bill with forty tickets available in Earplug.',
      tix: Ticketing.paid,
      cap: '40',
      ownerKind: GigOwnerKind.organization,
    ),
  ];

  static const bands = <String, Band>{
    'b1': Band(
      id: 'b1',
      name: 'Foghorn Diet',
      genres: ['garage', 'surf punk'],
      area: 'Mission, SF',
      color: Color(0xFF7B8FFF),
      initials: 'FD',
      followers: 486,
      reviewSummary: ReviewSummary(
        count: 1,
        mean: 5,
        completedBookings: 1,
        cancellations: 0,
      ),
      bio:
          'Reverb-soaked garage punk from a Mission garage that actually floods. '
          'Two guitars, no pedalboard budget, all heart.',
      profileComplete: true,
      upcoming: ['g2', 'g7'],
      past: [
        PastGig('Riptide warmup — Casa Quake', 'JUL 12'),
        PastGig('Nightcrawler in-store', 'JUN 27'),
        PastGig('Peralta Hall benefit', 'JUN 6'),
        PastGig('Bunker session', 'MAY 16'),
      ],
    ),
    'b2': Band(
      id: 'b2',
      name: 'Pigeon Court',
      genres: ['post-punk'],
      area: 'Temescal, Oakland',
      color: Color(0xFFB9C4FF),
      initials: 'PC',
      followers: 1214,
      bio:
          'Wiry post-punk for people who alphabetize their records and then fight about it.',
      profileComplete: true,
      upcoming: ['g2', 'g4', 'g7'],
      past: [
        PastGig('Court Is In Session tour', 'JUL'),
        PastGig('Noise Night Vol. 11', 'JUN 20'),
      ],
    ),
    'b3': Band(
      id: 'b3',
      name: 'Mission Creep',
      genres: ['hardcore'],
      area: 'Mission, SF',
      color: Color(0xFFE4DC4A),
      initials: 'MC',
      followers: 743,
      bio: 'Fast, short, gone. Sets under 20 minutes, guaranteed.',
      profileComplete: true,
      upcoming: ['g1', 'g4', 'g5'],
      past: [
        PastGig('Basement series #4', 'JUL 5'),
        PastGig('All-Ages Matinee', 'JUN 14'),
      ],
    ),
    'b4': Band(
      id: 'b4',
      name: 'Dial Tone Grief',
      genres: ['noise rock', 'shoegaze'],
      area: 'Bernal Heights, SF',
      color: Color(0xFF8FE6C4),
      initials: 'DT',
      followers: 312,
      bio:
          'Two amps facing each other, one long argument. You will feel it in your teeth.',
      profileComplete: true,
      upcoming: ['g1', 'g3', 'g7'],
      past: [PastGig('Noise Night Vol. 10', 'MAY 30')],
    ),
    'b5': Band(
      id: 'b5',
      name: 'Trash Panda Riot',
      genres: ['thrash', 'party punk'],
      area: 'West Oakland',
      color: Color(0xFFF0A26B),
      initials: 'TP',
      followers: 927,
      bio:
          'Party thrash with a raccoon on the kick drum. The raccoon is drawn on. Probably.',
      profileComplete: true,
      upcoming: ['g5', 'g6', 'g7'],
      past: [
        PastGig('Riot Fest (the small one)', 'JUL 4'),
        PastGig('Peralta Hall blowout', 'JUN 21'),
      ],
    ),
    'b6': Band(
      id: 'b6',
      name: 'Static Bloom',
      genres: ['shoegaze', 'punk'],
      area: 'Berkeley',
      color: Color(0xFFD9A6E8),
      initials: 'SB',
      followers: 158,
      bio:
          'Loud flowers. Berkeley basements and borrowed fuzz pedals since 2025.',
      profileComplete: true,
      upcoming: ['g1', 'g3', 'g5'],
      past: [PastGig('First show ever — house party', 'JUN 28')],
    ),
  };

  /// Band media for Foghorn Diet, including its clips and demo photos.
  static const b1Media = <BandMedia>[
    BandMedia(
      id: 'bm1',
      bandId: 'b1',
      kind: MediaKind.video,
      url: null,
      title: 'This is what we sound like — live at Foghorn Club',
      caption: null,
      sizeBytes: null,
      views: 12400,
      lengthSec: 161,
      pinned: true,
      order: 0,
      isHero: false,
    ),
    BandMedia(
      id: 'bm2',
      bandId: 'b1',
      kind: MediaKind.video,
      url: null,
      title: 'Riptide (practice take, one mic)',
      caption: null,
      sizeBytes: null,
      views: 3100,
      lengthSec: 185,
      pinned: false,
      order: 1,
      isHero: false,
    ),
    BandMedia(
      id: 'bm3',
      bandId: 'b1',
      kind: MediaKind.video,
      url: null,
      title: 'Undertow — basement set',
      caption: null,
      sizeBytes: null,
      views: 1800,
      lengthSec: 142,
      pinned: false,
      order: 2,
      isHero: false,
    ),
    BandMedia(
      id: 'bm4',
      bandId: 'b1',
      kind: MediaKind.video,
      url: null,
      title: 'New song, no name yet',
      caption: null,
      sizeBytes: null,
      views: 942,
      lengthSec: 107,
      pinned: false,
      order: 3,
      isHero: false,
    ),
    BandMedia(
      id: 'bm5',
      bandId: 'b1',
      kind: MediaKind.video,
      url: null,
      title: 'Soundcheck goes wrong (fun)',
      caption: null,
      sizeBytes: null,
      views: 5600,
      lengthSec: 58,
      pinned: false,
      order: 4,
      isHero: false,
    ),
    BandMedia(
      id: 'bm6',
      bandId: 'b1',
      kind: MediaKind.photo,
      url: null,
      title: 'Photo 1',
      caption: null,
      sizeBytes: null,
      views: null,
      lengthSec: null,
      pinned: false,
      order: 0,
      isHero: false,
    ),
    BandMedia(
      id: 'bm7',
      bandId: 'b1',
      kind: MediaKind.photo,
      url: null,
      title: 'Photo 2',
      caption: null,
      sizeBytes: null,
      views: null,
      lengthSec: null,
      pinned: false,
      order: 1,
      isHero: false,
    ),
  ];

  /// Generic band media shown for bands without their own uploaded clips.
  static const genericMedia = <BandMedia>[
    BandMedia(
      id: 'gm1',
      bandId: '_generic',
      kind: MediaKind.video,
      url: null,
      title: 'This is what we sound like',
      caption: null,
      sizeBytes: null,
      views: 1100,
      lengthSec: 130,
      pinned: true,
      order: 0,
      isHero: false,
    ),
    BandMedia(
      id: 'gm2',
      bandId: '_generic',
      kind: MediaKind.video,
      url: null,
      title: 'Live clip — last month',
      caption: null,
      sizeBytes: null,
      views: 640,
      lengthSec: 112,
      pinned: false,
      order: 1,
      isHero: false,
    ),
    BandMedia(
      id: 'gm3',
      bandId: '_generic',
      kind: MediaKind.video,
      url: null,
      title: 'Practice space take',
      caption: null,
      sizeBytes: null,
      views: 402,
      lengthSec: 153,
      pinned: false,
      order: 2,
      isHero: false,
    ),
    BandMedia(
      id: 'gm4',
      bandId: '_generic',
      kind: MediaKind.video,
      url: null,
      title: 'New one (rough)',
      caption: null,
      sizeBytes: null,
      views: 188,
      lengthSec: 79,
      pinned: false,
      order: 3,
      isHero: false,
    ),
    BandMedia(
      id: 'gm5',
      bandId: '_generic',
      kind: MediaKind.video,
      url: null,
      title: 'Full set audio',
      caption: null,
      sizeBytes: null,
      views: 871,
      lengthSec: 1324,
      pinned: false,
      order: 4,
      isHero: false,
    ),
  ];

  static const fanHistory = <PastGig>[
    PastGig('Basement series #4 — Casa Quake', 'JUL 5'),
    PastGig('Noise Night Vol. 11 — Nightcrawler', 'JUN 20'),
    PastGig('All-Ages Matinee — Peralta Hall', 'JUN 14'),
    PastGig('Peralta Hall benefit', 'JUN 6'),
  ];
}
