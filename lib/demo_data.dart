import 'package:flutter/material.dart';
import 'package:latlong2/latlong.dart';

import 'models.dart';

/// Demo dataset lifted verbatim from the design spec. This is the seam where
/// Convex queries will plug in later — screens only ever see these shapes.
abstract final class DemoData {
  static const venues = <String, Venue>{
    'v1': Venue(
        id: 'v1',
        name: 'The Foghorn Club',
        area: 'Mission, SF',
        addr: '2455 Harrison St, San Francisco',
        distSF: '0.8 mi',
        distOak: '6.3 mi',
        point: LatLng(37.7524, -122.4180)),
    'v2': Venue(
        id: 'v2',
        name: 'Nightcrawler Records',
        area: 'Temescal, Oakland',
        addr: '486 40th St, Oakland',
        distSF: '6.1 mi',
        distOak: '0.9 mi',
        point: LatLng(37.8180, -122.2690)),
    'v3': Venue(
        id: 'v3',
        name: 'Casa Quake',
        area: 'Bernal Heights, SF',
        addr: '(address with RSVP)',
        distSF: '1.2 mi',
        distOak: '6.7 mi',
        point: LatLng(37.7399, -122.4166)),
    'v4': Venue(
        id: 'v4',
        name: 'The Rusty Anchor',
        area: 'Dogpatch, SF',
        addr: '701 22nd St, San Francisco',
        distSF: '1.9 mi',
        distOak: '5.4 mi',
        point: LatLng(37.7583, -122.3880)),
    'v5': Venue(
        id: 'v5',
        name: 'Peralta Hall',
        area: 'West Oakland',
        addr: '1801 Peralta St, Oakland',
        distSF: '6.8 mi',
        distOak: '1.6 mi',
        point: LatLng(37.8060, -122.2910)),
    'v6': Venue(
        id: 'v6',
        name: 'Sunset Bunker',
        area: 'Outer Sunset, SF',
        addr: '3820 Noriega St, San Francisco',
        distSF: '4.3 mi',
        distOak: '9.8 mi',
        point: LatLng(37.7540, -122.5040)),
  };

  static const flyers = <String, FlyerStyle>{
    'paper': FlyerStyle(
        base: Color(0xFFF4F4F0),
        stripe: Color(0x0D000000),
        fg: Color(0xFF111114)),
    'blue': FlyerStyle(
        base: Color(0xFF1435F0),
        stripe: Color(0x1F000000),
        fg: Color(0xFFFFFFFF)),
    'black': FlyerStyle(
        base: Color(0xFF141418),
        stripe: Color(0x0DFFFFFF),
        fg: Color(0xFFF4F4F0)),
    'yellow': FlyerStyle(
        base: Color(0xFFE4DC4A),
        stripe: Color(0x0F000000),
        fg: Color(0xFF111114)),
    'bluetype': FlyerStyle(
        base: Color(0xFFF4F4F0),
        stripe: Color(0x141435F0),
        fg: Color(0xFF1435F0)),
  };

  static const gigs = <Gig>[
    Gig(
        id: 'g1',
        title: 'Basement Blowout',
        venueId: 'v3',
        price: 0,
        dateShort: 'TUE JUL 28',
        dateLine: 'TONIGHT · DOORS 8PM',
        time: '8PM / 9PM',
        when: GigWhen.tonight,
        flyKey: 'paper',
        lineup: ['b3', 'b4', 'b6'],
        going: 43,
        genres: ['hardcore', 'noise'],
        desc: 'Four bands, one basement, zero cover. Bring earplugs (the foam kind).',
        tix: Ticketing.rsvp),
    Gig(
        id: 'g2',
        title: 'Riptide Release Show',
        venueId: 'v1',
        price: 10,
        dateShort: 'THU JUL 30',
        dateLine: 'THU · DOORS 8PM',
        time: '8PM / 9PM',
        when: GigWhen.week,
        flyKey: 'blue',
        lineup: ['b1', 'b2'],
        going: 87,
        genres: ['garage', 'surf'],
        desc: "Foghorn Diet plays 'Riptide' front to back, then whatever falls out.",
        tix: Ticketing.rsvp),
    Gig(
        id: 'g3',
        title: 'Noise Night Vol. 12',
        venueId: 'v2',
        price: 8,
        dateShort: 'FRI JUL 31',
        dateLine: 'FRI · DOORS 9PM',
        time: '9PM / 9:30PM',
        when: GigWhen.week,
        flyKey: 'black',
        lineup: ['b4', 'b6'],
        going: 29,
        genres: ['noise', 'shoegaze'],
        desc: 'In-store racket between the record bins. BYO earplugs, again.',
        tix: Ticketing.rsvp),
    Gig(
        id: 'g4',
        title: 'Pigeon Court + Mission Creep',
        venueId: 'v4',
        price: 12,
        dateShort: 'SAT AUG 1',
        dateLine: 'SAT · DOORS 8PM',
        time: '8PM / 8:45PM',
        when: GigWhen.week,
        flyKey: 'bluetype',
        lineup: ['b2', 'b3'],
        going: 56,
        genres: ['post-punk', 'hardcore'],
        desc: 'Double bill. Loud then louder. Tickets through the venue.',
        tix: Ticketing.external),
    Gig(
        id: 'g5',
        title: 'All-Ages Matinee',
        venueId: 'v5',
        price: 0,
        dateShort: 'SUN AUG 2',
        dateLine: 'SUN · 2PM SHARP',
        time: '1:30PM / 2PM',
        when: GigWhen.week,
        flyKey: 'yellow',
        lineup: ['b5', 'b6', 'b3'],
        going: 61,
        genres: ['punk'],
        desc: 'Daylight punk. Kids welcome, chaperones tolerated.',
        tix: Ticketing.rsvp),
    Gig(
        id: 'g6',
        title: 'Trash Panda Riot',
        venueId: 'v6',
        price: 5,
        dateShort: 'FRI AUG 7',
        dateLine: 'FRI · DOORS 9PM',
        time: '9PM / 10PM',
        when: GigWhen.later,
        flyKey: 'black',
        lineup: ['b5'],
        going: 18,
        genres: ['thrash', 'punk'],
        desc: 'One band, ninety minutes, a bunker by the beach.',
        tix: Ticketing.rsvp),
    Gig(
        id: 'g7',
        title: 'Fog City Fest — Day Show',
        venueId: 'v1',
        price: 15,
        dateShort: 'SUN AUG 9',
        dateLine: 'SUN · NOON–8PM',
        time: '12PM / 12:30PM',
        when: GigWhen.later,
        flyKey: 'blue',
        lineup: ['b1', 'b2', 'b4', 'b5'],
        going: 112,
        genres: ['punk', 'garage'],
        desc: 'Eight hours, four stages worth of bands on one stage.',
        tix: Ticketing.external),
  ];

  static const bands = <String, Band>{
    'b1': Band(
        name: 'Foghorn Diet',
        genres: ['garage', 'surf punk'],
        area: 'Mission, SF',
        color: Color(0xFF7B8FFF),
        initials: 'FD',
        followers: 486,
        bio: 'Reverb-soaked garage punk from a Mission garage that actually floods. '
            'Two guitars, no pedalboard budget, all heart.',
        upcoming: ['g2', 'g7'],
        past: [
          PastGig('Riptide warmup — Casa Quake', 'JUL 12'),
          PastGig('Nightcrawler in-store', 'JUN 27'),
          PastGig('Peralta Hall benefit', 'JUN 6'),
          PastGig('Bunker session', 'MAY 16'),
        ]),
    'b2': Band(
        name: 'Pigeon Court',
        genres: ['post-punk'],
        area: 'Temescal, Oakland',
        color: Color(0xFFB9C4FF),
        initials: 'PC',
        followers: 1214,
        bio: 'Wiry post-punk for people who alphabetize their records and then fight about it.',
        upcoming: ['g2', 'g4', 'g7'],
        past: [
          PastGig('Court Is In Session tour', 'JUL'),
          PastGig('Noise Night Vol. 11', 'JUN 20'),
        ]),
    'b3': Band(
        name: 'Mission Creep',
        genres: ['hardcore'],
        area: 'Mission, SF',
        color: Color(0xFFE4DC4A),
        initials: 'MC',
        followers: 743,
        bio: 'Fast, short, gone. Sets under 20 minutes, guaranteed.',
        upcoming: ['g1', 'g4', 'g5'],
        past: [
          PastGig('Basement series #4', 'JUL 5'),
          PastGig('All-Ages Matinee', 'JUN 14'),
        ]),
    'b4': Band(
        name: 'Dial Tone Grief',
        genres: ['noise rock', 'shoegaze'],
        area: 'Bernal Heights, SF',
        color: Color(0xFF8FE6C4),
        initials: 'DT',
        followers: 312,
        bio: 'Two amps facing each other, one long argument. You will feel it in your teeth.',
        upcoming: ['g1', 'g3', 'g7'],
        past: [PastGig('Noise Night Vol. 10', 'MAY 30')]),
    'b5': Band(
        name: 'Trash Panda Riot',
        genres: ['thrash', 'party punk'],
        area: 'West Oakland',
        color: Color(0xFFF0A26B),
        initials: 'TP',
        followers: 927,
        bio: 'Party thrash with a raccoon on the kick drum. The raccoon is drawn on. Probably.',
        upcoming: ['g5', 'g6', 'g7'],
        past: [
          PastGig('Riot Fest (the small one)', 'JUL 4'),
          PastGig('Peralta Hall blowout', 'JUN 21'),
        ]),
    'b6': Band(
        name: 'Static Bloom',
        genres: ['shoegaze', 'punk'],
        area: 'Berkeley',
        color: Color(0xFFD9A6E8),
        initials: 'SB',
        followers: 158,
        bio: 'Loud flowers. Berkeley basements and borrowed fuzz pedals since 2025.',
        upcoming: ['g1', 'g3', 'g5'],
        past: [PastGig('First show ever — house party', 'JUN 28')]),
  };

  static const b1Videos = <VideoClip>[
    VideoClip(
        title: 'This is what we sound like — live at Foghorn Club',
        views: '12.4K views',
        len: '2:41',
        pinned: true),
    VideoClip(title: 'Riptide (practice take, one mic)', views: '3.1K views', len: '3:05'),
    VideoClip(title: 'Undertow — basement set', views: '1.8K views', len: '2:22'),
    VideoClip(title: 'New song, no name yet', views: '942 views', len: '1:47'),
    VideoClip(title: 'Soundcheck goes wrong (fun)', views: '5.6K views', len: '0:58'),
  ];

  /// Generic clip list shown for bands that haven't uploaded (everyone but b1).
  static const genericVideos = <VideoClip>[
    VideoClip(title: 'This is what we sound like', views: '1.1K views', len: '2:10', pinned: true),
    VideoClip(title: 'Live clip — last month', views: '640 views', len: '1:52'),
    VideoClip(title: 'Practice space take', views: '402 views', len: '2:33'),
    VideoClip(title: 'New one (rough)', views: '188 views', len: '1:19'),
    VideoClip(title: 'Full set audio', views: '871 views', len: '22:04'),
  ];

  static const fanHistory = <PastGig>[
    PastGig('Basement series #4 — Casa Quake', 'JUL 5'),
    PastGig('Noise Night Vol. 11 — Nightcrawler', 'JUN 20'),
    PastGig('All-Ages Matinee — Peralta Hall', 'JUN 14'),
    PastGig('Peralta Hall benefit', 'JUN 6'),
  ];
}
